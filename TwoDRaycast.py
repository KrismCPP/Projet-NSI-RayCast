...
class Wall :
    def __init__(self,Point1,Point2,color=None) -> None:
        self.point1 = Point1
        self.point2 = Point2
        self.color = color
...
class POV :
    def __init__(self, FOV, Game) -> None:
        self.position = Vector2D(Game.RealMap.scale//2,Game.RealMap.scale//2)
        self.camAngle = 0
        self.FOV = FOV
        self.Game = Game
    ...
    def scanWalls(self) :
        ...
        for i in range (-self.FOV//2 , self.FOV//2) :
            ...
            Ray.rotateRay(angle*(pi/180))
            scans += [Ray.intersection(self.Game.RealMap)[1]]
        return scans
