...
def init_graphic(W,H,name="Fenêtre",bg_color=black,fullscreen=0) :
    """
...
def draw_pixel(x,y,C) :
    ...
    """
    x = int(x); y = int(y)
    pygame.draw.line(PYGAME_SDL_WINDOW,C,(x,y),(x,y),1)
...
def load_image_2(F,x,y) :
    ...
    x = int(x); y = int(y)
    fond = pygame.image.load(F).convert()
    PYGAME_SDL_WINDOW.blit(fond,(x,y))
    return fond
